...
if (isset($_GET['transaksi'])) {
    $id = $_GET['transaksi'];
    $query = mysqli_query($conn, "SELECT * FROM tambah_rm JOIN kamar ON(kamar.id_kamar = tambah_rm.id_inap) JOIN rawat_inap ON(rawat_inap.id_ri = kamar.kelas) WHERE id_rm = '$id'");
    ...
    $id_inap = $result['id_inap'];
    $tgl_masuk = new DateTime($result['tanggal']);
    $tgl_keluar = new DateTime($result['tanggal_rm']);
    $jarak = $tgl_keluar->diff($tgl_masuk);
    $harga_penginapan = $jarak->d * $result['harga_kamar'];
    $dokter = 200000;
    ...
    if ($row == 0) {
        ...
        document.location.href = 'tambah-farmasih.php?proses=" . $id . "';</script>";
    }
}
...
if (isset($_POST['simpan'])) {
    ...
    $query = mysqli_query($conn, "INSERT INTO transaksi2 (id_psinap, nama_pasien, diagnosis, tindakan, total_tagihan, total_obat, uang_bayar, tanggal_transaksi) VALUES('$id_inap','$data_pasien', '$data_diagnosis', '$data_tindakan', '$total_tagihan', '$total_obat', '$uang_bayar', '$tanggal_transaksi')");
    if ($query) {
        $update = mysqli_query($conn, "UPDATE tambah_rm SET status = 'selesai' WHERE id_rm = '$id';");
        header('location:struk_inap.php');
    };
};
?>
                ...
                <form method="POST" action="" enctype="multipart/form-data">
                    ...
                    <div class="container" style="margin-left: 100px;">
                        ...
                        <div>
                            ...
                            <input type="hidden" name="id_inap" value="<?php echo $id_inap ?>">
