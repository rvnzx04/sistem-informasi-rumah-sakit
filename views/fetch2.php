...
$id = $_GET['id'];
$query = mysqli_query($conn, "SELECT * FROM tambah_berobat JOIN spesialis ON (spesialis.id_spesialis = tambah_berobat.spesialis) WHERE id_berobat = $id");
                ...
                <?php foreach ($query as $row) { ?>
                    <form method="post" action="" enctype="multipart/form-data">
                        <tr class="text-black">
                            ...
                            <td><?= $row['nama'] ?></td>
                            <input type="hidden" name="spesialis" value="<?= $row['nama_spesialis'] ?>">
...
if (isset($_POST['kirim'])) {
    ...
    $query = "INSERT INTO tambah_pb VALUES(null, '$nik', '$nama', '$spesialis', '$tanggal', 'pending')";
    $sql = mysqli_query($conn, $query);
    if ($query) {
        $_SESSION['eksekusi'] = "Pasien Berobat Berhasil Dikirim";
